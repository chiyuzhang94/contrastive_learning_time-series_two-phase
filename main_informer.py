...
from data.utils import pkl_save
...
parser.add_argument('--freeze_encoder', type=str, default="False", help='whether to freeze encoder at fine-tuning time')
parser.add_argument('--svm_evaluate', type=str, default="False", help='whether use svm for forecasting')
parser.add_argument('--mare', type=str, default="False", help='whether use mare')
...
args.freeze_encoder = bool_change(args.freeze_encoder)
args.svm_evaluate = bool_change(args.svm_evaluate)
...
for ii in range(args.itr):
    ...
    if args.model=='informer' or args.model=='informerstack':
    ...
    elif args.model=='lstm' or args.model=='tcn' or args.model=='dtcn':
        ...
        if args.svm_evaluate:
            setting = setting = 'contrast-{}_{}_contrastL{}_freezeen{}_svm{}_mare{}_mask{}_l2norm{}_ft{}_avg{}_cos{}_aug{}_{}_{}'.format(
                    args.model, args.data, args.loss_lambda, str(args.freeze_encoder), str(args.svm_evaluate), str(args.mare),
                    args.mask_rate, str(args.l2norm), args.features, 
                    str(args.moco_average_pool), str(args.cos_lr), args.data_aug, args.des, ii)
        else:
            setting = 'contrast-{}_{}_contrastL{}_freezeen{}_svm{}_mare{}_mask{}_l2norm{}_ft{}_sl{}_pl{}_dm{}_el{}_avg{}_cos{}_aug{}_{}_{}'.format(
                    args.model, args.data, args.loss_lambda, str(args.freeze_encoder), str(args.svm_evaluate), str(args.mare),
                    args.mask_rate, str(args.l2norm), args.features, 
                    args.seq_len, args.pred_len,
    ...
    if args.svm_evaluate:
        out, eval_res = eval_forecasting(exp, args)
        out_path = args.des_path + '/' + setting
        pkl_save(f'{out_path}/eval_res.pkl', eval_res)
        print('Evaluation result:', eval_res)
    else:
        print('>>>>>>>start fine-tuning : {}>>>>>>>>>>>>>>>>>>>>>>>>>>'.format(setting))
        exp.train_task(setting)
